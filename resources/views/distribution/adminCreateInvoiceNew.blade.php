@php
    $privilageId = \DB::table('pm_interfaces')
        ...
        ->where('pm_interfaces.path', 'adminCreateInvoiceNew')
        ->first();
@endphp
...
<style>
    ...
    <div class="row wrapper border-bottom white-bg page-heading">
        <div class="col-lg-12">
            <h2><b>Create Invoice <small>(Retail Price)</small></b></h2>
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item active">
                    <strong>Create Invoice <small>(Retail Price)</small></strong>
                </li>
            </ol>
        </div>
    </div>
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    ...
                    <button type="button" class="btn btn-warning btn-sm" onclick="saveInvoiceData()" style="background-color: #c81bd8; border-color: #c81bd8;">
                        <i class="fa fa-print" aria-hidden="true"></i> &nbsp; Generate Invoice
                    </button>
                </div>
            </div>
        </div>
    ...
    <script>
            ...
            if (customer == 0) {
                ...
                jQuery.ajax({
                    ...
                    data: {
                        ...
                        "url": "distribution.ajaxInvoice.ajaxInvoiceLoadTable_new",
                    },
                    beforeSend: function() {
                        showLder();
                    },
            ...
            if (rowCount == 0) {
                ...
                jQuery.ajax({
                    ...
                    data: {
                        ...
                        "displayDiscount": 0.0,
                        "specialDiscount": 0.0,
                        "customDiscount": Number($('#genCustomDiscount').text().replace(/,/g, '')),
                        "customDiscountPercentage": $("#custom_discount").val()
                    },
                    beforeSend: function() {
                        showLder();
                    },
        ...
        function calculateTotal() {
            ...
            for (var i = 1; i < table.rows.length; i++) {
                ...
                let nonDisValue = parseFloat($("#qtyTot" + i).text().replace(/,/g, ''));
                if (nonDisValue > 0 &&
                    (productSubCategory_id === 51 || // 51 = Noodles 400g --> [6]
                     productSubCategory_id === 57 || // 57 = Noodles 5kg --> [6]
                     productSubCategory_id === 59 || // 59 = Kiri aluwa bottle
                     productSubCategory_id === 60 || // 60 = Thala kiri aluwa bottle
                     productSubCategory_id === 61 || // 61 = Mix kiri aluwa bottle
                     productSubCategory_id === 63 )  // 63 = Kiri Toffee Bottle
                ) {
                    nonDiscountableProductsTotalPrice += nonDisValue;
                }
            }

            // console.log('sumVal -- '+sumVal);
            // console.log('sumIssueVal -- '+sumIssueVal);
            ...
            var customDiscount = 0.0;

            // calculate Custom Discount by selecting Rate
            if ($("#custom_discount").val() > 0 && sumVal > 0) {
                // customDiscount = (parseFloat(grossAmount - nonDiscountableProductsTotalPrice) * $("#custom_discount").val()) / 100;
                customDiscount = ( parseFloat(sumVal) * parseFloat($("#custom_discount").val()) ) / 100;
            } else {
                customDiscount = 0.0;
            }

            allSubDis = parseFloat(grossAmount) - parseFloat(customDiscount);
            ...
            $("#genCustomDiscount").text(customDiscount.toLocaleString("en-US", {minimumFractionDigits: 2, maximumFractionDigits: 2})); // Custom Discount
            $("#genTotal").text(finalTotal.toLocaleString("en-US", {minimumFractionDigits: 2, maximumFractionDigits: 2})); // Total
        }
