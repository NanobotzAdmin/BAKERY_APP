        ...
        <table class="table sticky table-hover styled-table" style="width: 1300px;" id="TblAddReturnToTBL">
            <thead>
                <tr style="text-align: center;">
                    ...
                    <th style="min-width: 160px; width: 180px;">Unit Price</th>
                ...
                @foreach ($deliveryProductData as $products)
                    ...
                    <tr class="product" product-main-category="{{ $productSubCategory->pm_product_main_category_id }}" product-sub-category="{{ $productSubCategory->id }}">
                        ...
                        <td>
                            ...
                            <label>Price</label>
                            <input type="hidden" name="hideReturnPrice" id="hideReturnPrice{{ $id }}" value="{{ $stock->selling_price }}" />
                            ...
                            <select class="form-control" onchange="validateReturn({{ $id }})" id="returnCombo{{ $id }}">
                                <option value={{ $stock->selling_price }} selected>{{ $stock->selling_price }} - Selling Price</option>
                        ...
                        <td style="vertical-align: middle; text-align: center;"> <input type="text" class="form-control form-control-sm sellingPriceCss" value="{{ $stock->selling_price }}" name="sellingPrice" id="sellingPrice{{ $id }}" disabled /></td>
                        ...
                        <td style="display: none"><input type="text" value="{{ $stock->selling_price }}" name="sellingPriceNormal" id="sellingPriceNormal{{ $id }}" disabled /></td>

                        <td style="display: none">{{ $productSubCategory->id }}</th>
                        <td id="IssueTot{{ $id }}" style="display: none">0.0</td>
                        <td id="returnTot{{ $id }}" style="display: none">0.0</td>
    ...
    function viewReturnModal() {
        ...
        jQuery.ajax({
            ...
            data: {
                ...
                "url": "distribution.ajaxInvoice.ajaxLoadProductsToReturn",
            },
            beforeSend: function() {
                showLder();
            },
