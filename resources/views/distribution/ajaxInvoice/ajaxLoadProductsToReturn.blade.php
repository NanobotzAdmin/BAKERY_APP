<div class="row">
        <div class="form-row">
                ...
                <select class="form-control" id="productsReturn" onchange="loadProductId()">
                    ...
                    <option value="{{ $product->id }},{{ $product->selling_price }},{{ $product->discountable_qty }},{{ $product->discounted_price }}">{{ $product->sub_category_name }}</option>
                    @endforeach
...
var sellingPrice= productArr[1];
var discounted = productArr[3];
$("#returnProId").val(proId);
$("#ReturnQty").val(0);
$("#hiddenReturnUnitPrice").val(0);
$("#ReturnPrice2").val(0);
  $('#returnPriceCombo').append(`<option value="select">Select One</option><option value="${sellingPrice}">${sellingPrice} - Selling Price</option><option value="${discounted}">${discounted} - Discounted Price</option>`)
